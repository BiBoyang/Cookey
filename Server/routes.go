...
func writeJumpHTML(w http.ResponseWriter, deepLink string) {
	...
	io.WriteString(w, `:root{color-scheme:light dark;--bg:#fff;--ink:#171717;--muted:#737373;--accent:#22c55e;--accent-rgb:34,197,94;--btn-bg:#171717;--btn-fg:#fff}@media(prefers-color-scheme:dark){:root{--bg:#0a0a0a;--ink:#f0f0f0;--muted:#888;--accent:#4ade80;--accent-rgb:74,222,128;--btn-bg:#f0f0f0;--btn-fg:#0a0a0a}}*{margin:0;box-sizing:border-box}body{min-height:100vh;min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--bg);color:var(--ink);font:16px/1.6 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}main{text-align:center}h1{font-size:22px;font-weight:600;line-height:1.2;margin-bottom:8px}p{color:var(--muted);font-size:15px}a.btn{display:inline-flex;align-items:center;gap:6px;margin-top:24px;padding:12px 24px;border-radius:10px;background:var(--btn-bg);color:var(--btn-fg);text-decoration:none;font-size:15px;font-weight:600;transition:opacity .15s}a.btn:active{opacity:.7}a.btn svg{width:16px;height:16px;fill:var(--btn-fg)}.sub{margin-top:16px;font-size:13px}.dot{width:10px;height:10px;margin:0 auto 20px;border-radius:999px;background:var(--accent);animation:pulse 2s cubic-bezier(.4,0,.6,1) infinite}@keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(var(--accent-rgb),.4)}50%{box-shadow:0 0 0 14px rgba(var(--accent-rgb),0)}}</style>`)
	io.WriteString(w, `</head><body><main><div class="dot"></div><h1>Opening Cookey…</h1><p id="status">Tap below if the app doesn&rsquo;t open automatically.</p>`)
	io.WriteString(w, `<a class="btn" id="open-link" href="`)
	io.WriteString(w, escapedLink)
	io.WriteString(w, `"><svg viewBox="0 0 16 16"><path d="M3.5 9.9a.75.75 0 0 1-1.5 0V4.75A2.75 2.75 0 0 1 4.75 2h6.5A2.75 2.75 0 0 1 14 4.75v5.15a.75.75 0 0 1-1.5 0V4.75c0-.69-.56-1.25-1.25-1.25h-6.5c-.69 0-1.25.56-1.25 1.25ZM8.53 14.53a.75.75 0 0 1-1.06 0l-3.25-3.25a.75.75 0 1 1 1.06-1.06L7.5 12.44V6.75a.75.75 0 0 1 1.5 0v5.69l2.22-2.22a.75.75 0 1 1 1.06 1.06Z"/></svg>Open Cookey</a><p class="sub">You can close this tab after opening the app.</p>`)
